...
const deps = {
  ...
  jwtVerify: (token: string, _getKey: any, _opts: any, cb: (err: Error | null, decoded?: unknown) => void) => {
...
describe('verifyJwtWithDeps', () => {
  it('resolves decoded payload on success', async () => {
    const decoded = await verifyJwtWithDeps('good', { issuer: 'iss', audience: 'aud', jwksUri: 'https://jwks' }, deps)
    expect(decoded.sub).toBe('user-1')
  })

  it('rejects on token error', async () => {
    await expect(verifyJwtWithDeps('bad', { issuer: 'iss', audience: 'aud', jwksUri: 'https://jwks' }, deps))
  ...
  it('rejects on missing kid', async () => {
    ...
    await expect(verifyJwtWithDeps('good', { issuer: 'iss', audience: 'aud', jwksUri: 'https://jwks' }, depsMissingKid as any)).rejects.toThrow('Missing kid')
  })
})
