...
export function HomePage() {
  ...
  return (
    <div className="grid gap-6">
      {/* Hero */}
      <section className="rounded-xl border border-slate-200 p-6" style={{ background: 'linear-gradient(120deg, rgba(106,227,255,0.10), rgba(167,139,250,0.10))' }}>
        <div className="max-w-3xl">
          ...
          <div className="flex gap-3">
            <a href="#search"><Button>Start Exploring</Button></a>
            <Link href="/subscriptions"><Button variant="secondary">Connect Services</Button></Link>
          </div>
        </div>
      </section>
      ...
      <Card id="search">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3 items-end">
          ...
          <div>
            ...
            <Select
              value={service}
              onChange={(e) => { setService(e.target.value); setFrom(0); }}
              className="mt-1"
            >
          ...
          <div>
            ...
            <Select
              value={region}
              onChange={(e) => { setRegion(e.target.value); setFrom(0); }}
              className="mt-1"
            >
              <option value="">Any</option>
              <option>US</option>
        ...
        <div className="mt-3 flex gap-3">
          <Button onClick={() => { setFrom(0); runSearch(); }}>Search</Button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {items.map((it) => (
            <Card key={it.id} className="hover:shadow-sm">
              <div className="flex gap-3">
                <Thumb posterUrl={it.posterUrl} backdropUrl={it.backdropUrl} className="w-16 h-24 rounded-md overflow-hidden flex-shrink-0" />
                <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {trending.map((it) => (
            <Card key={`t-${it.id}`} className="hover:shadow-sm">
              <Thumb posterUrl={it.posterUrl} backdropUrl={it.backdropUrl} className="w-full h-32 rounded-md overflow-hidden mb-3" />
              <div className="text-base font-semibold">{it.name}</div>
              <div className="text-sm text-slate-500">{it.type || 'Unknown'} {it.releaseYear ? `• ${it.releaseYear}` : ''}
                {typeof it.voteAverage === 'number' ? (
                  <span className="ml-2 text-amber-400">★ {it.voteAverage.toFixed(1)}</span>
                ) : null}
              </div>
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card>
          ...
          <p className="text-sm text-slate-500">Find movies and shows across all your streaming services, instantly.</p>
        </Card>
        <Card>
          <div className="text-2xl mb-2">✨</div>
          <h3 className="text-base font-medium mb-1">Daily Picks</h3>
          <p className="text-sm text-slate-500">Smart, fresh recommendations tailored to your subscriptions and taste.</p>
        </Card>
        <Card>
          <div className="text-2xl mb-2">🔔</div>
          <h3 className="text-base font-medium mb-1">Availability Alerts</h3>
          <p className="text-sm text-slate-500">Get notified the moment a title becomes available where you watch.</p>
        </Card>
      </section>
    </div>
  )
