...
export async function ensureIndex(indexName: string, mapping: object): Promise<void> {
  const existsRes = await fetch(`${DEFAULT_OS_URL}/${encodeURIComponent(indexName)}`, { method: 'HEAD' });
...
export async function indexDocument(indexName: string, id: string, doc: unknown): Promise<void> {
  const res = await fetch(`${DEFAULT_OS_URL}/${encodeURIComponent(indexName)}/_doc/${encodeURIComponent(id)}`, {
    method: 'PUT',
    headers: { 'content-type': 'application/json' },
